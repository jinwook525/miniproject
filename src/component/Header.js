...
export default function Header({ onLoginClick, onSignUpClick, onBoardClick }) {
  ...
  return (
    <div className="w-full bg-white border-b-2 border-black flex flex-col items-center h-4/5">
      {/* 상단 버튼 */}
        ...
        {isAuthenticated ? (
          <>
            ...
            <span className="mr-4 text-black font-bold">
              <span className="text-red-500">{nickname}</span>님 환영합니다!
            </span>
            <button
              onClick={handleLogout}
              className="mr-4 cursor-pointer text-black hover:text-red-600 transition duration-200 font-bold text-lg"
            >
              로그아웃
            </button>
          </>
          ...
          <>
            ...
            <button
              ...
              className="mr-4 cursor-pointer text-black hover:text-blue-600 transition duration-200 font-bold text-lg"
            >
              로그인
            </button>
            <button
      ...
      <div className="h-2/3 flex justify-center my-4">
        <div className="text-4xl font-cafe24 font-extrabold text-gray-800">Bugsynergy</div>
      </div>
      ...
      <div className="flex justify-center space-x-10 py-8">
        <button
          onClick={() => navigate('/')}
          className="cursor-pointer text-black hover:text-blue-600 transition duration-200 font-bold text-xl"
        >
          레시피
        </button>
